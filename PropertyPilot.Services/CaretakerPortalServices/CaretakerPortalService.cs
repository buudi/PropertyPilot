...
public class CaretakerPortalService(PmsDbContext pmsDbContext, FinancesService financesService, PropertiesService propertiesService, TenantService tenantService)
{
    ...
    public async Task<PaginatedResult<InvoicesTabListing>> GetPropertiesInvoicesTabAsync(Guid propertyId, int pageSize, int pageNumber)
    {
        ...
        foreach (var invoiceRecord in invoiceRecords)
        {
            ...
            var totalAmount = await invoiceRecord.Invoice.TotalAmountMinusDiscountAsync(pmsDbContext);

            var amountDue = await invoiceRecord.Invoice.TotalAmountRemainingAsync(pmsDbContext);
    ...
    public async Task<PaginatedResult<InvoicesTabListing>> GetTenancyInvoicesTabAsync(Guid tenancyId, int pageSize, int pageNumber)
    {
        var totalItems = await pmsDbContext
            .Invoices
    ...
    public async Task<PaginatedResult<PaymentsTabListing>> GetTenancyPaymentsTabAsync(Guid tenancyId, int pageSize, int pageNumber)
    {
        var invoicesIds = await pmsDbContext
            .Invoices
            .Where(x => x.TenancyId == tenancyId)
